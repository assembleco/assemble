...
unless defined?(Rails::Console) ||
  ...
  schedule.cron "0 0 * * 0" do
    ...
    Rails.logger.flush
    Event.create(feed: Feed.find_by!(name: "weekly"), data: { time: Time.now })
  end
  ...
  schedule.cron "0 0 * * *" do
    ...
    Rails.logger.flush
    Event.create(feed: Feed.find_by!(name: "daily"), data: { time: Time.now })
  end
  ...
  schedule.cron "0 * * * *" do
    ...
    Rails.logger.flush
    Event.create(feed: Feed.find_by!(name: "hourly"), data: { time: Time.now })
  end
  ...
  schedule.cron "* * * * *" do
    ...
    Rails.logger.flush
    Event.create(feed: Feed.find_by!(name: "minutely"), data: { time: Time.now })
  end
end
